...
This queue is finite for v1. A task is complete only with executable code and validation evidence.
...
4. Close one complete task before starting the next; a local failure does not cancel the remaining queue.
...
| 8 | Backend integration | retarget to release main + exact-head CI | PENDING |
...
## Rule
A failed subtask produces a correction and a new validation cycle; it never resets completed gates and never expands v1 indefinitely.
