...
// MessageFromMesh adapts the canonical SOUL-style mesh envelope into N07's protocol.
...
func MeshFromMessage(m Message) (MeshEnvelope,error) {
	...
	if len(m.PayloadJSON)>0 {if err:=json.Unmarshal(m.PayloadJSON,&payload);err!=nil{return MeshEnvelope{},err}}
	if len(m.Payload)>0 {payload["values"]=append([]float64(nil),m.Payload...)}
	return MeshEnvelope{ContractVersion:Version,Operation:m.Operation,Payload:payload,CorrelationID:m.TraceID,Source:m.Source,Target:m.Target,Metadata:cloneMetadata(m.Metadata)},nil
}
