...
type Metrics struct {
	...
	Failures Counter
	mu sync.RWMutex
	byComponent map[string]*TaskMetric
}
...
func TraceID(ctx context.Context)string{if ctx==nil{return ""};type traceKey struct{};if v:=ctx.Value(traceKey{});v!=nil{if s,ok:=v.(string);ok{return s}};return ""}
