...
func (p *SARAControlPublisher) Publish(ctx context.Context, event VagusEnvelope) error {
	if p == nil || p.Status() == "UNCONFIGURED" {
		return errors.New("VAGUS_CONTROL_UNCONFIGURED")
	}
	if ctx == nil {
	...
	if err != nil {
		return fmt.Errorf("encode Vagus envelope: %w", err)
	}
	...
	if err != nil {
		return fmt.Errorf("Vagus control transport: %w", err)
	}
	defer resp.Body.Close()
	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
		return fmt.Errorf("Vagus control HTTP %d", resp.StatusCode)
	}
	return nil
}
