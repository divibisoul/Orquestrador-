...
func RegisterOctaCoreOperations(engine *orchestrator.Engine, scheduler *OctaCoreScheduler) error {
    ...
    for name, handler := range registrations { if err := engine.Register(name, handler); err != nil { return err } }
    return nil
}

func decodeJob(metadata map[string]string) (OctaCoreJob, error) {
    raw := strings.TrimSpace(metadata["octacore_job_json"])
    if raw == "" { return OctaCoreJob{}, errors.New("metadata.octacore_job_json is required") }
    var job OctaCoreJob
    if err := json.Unmarshal([]byte(raw), &job); err != nil { return OctaCoreJob{}, err }
    if err := job.Validate(); err != nil { return OctaCoreJob{}, err }
    return job, nil
}

func decodeJobs(metadata map[string]string) ([]OctaCoreJob, error) {
    raw := strings.TrimSpace(metadata["octacore_jobs_json"])
    if raw == "" { return nil, errors.New("metadata.octacore_jobs_json is required") }
    var jobs []OctaCoreJob
    if err := json.Unmarshal([]byte(raw), &jobs); err != nil { return nil, err }
    if len(jobs) == 0 { return nil, errors.New("Octacore batch is empty") }
    for i, job := range jobs { if err := job.Validate(); err != nil { return nil, fmt.Errorf("job %d: %w", i, err) } }
    return jobs, nil
}
...
func octaProtocolResult(message protocol.Message, raw []byte, err error) (protocol.Result, error) {
    ...
    if len(raw) > 0 { metadata["octacore_json"] = string(raw) }
    ...
    if err != nil { result.Status="error"; result.Error=err.Error(); return result, err }
    return result, nil
}
