...
func NewProcessor(cfg SchedulerConfig, control ControlPublisher, compute interface{ Discover() []struct{} }) (*Processor, error) {
    _ = compute
    scheduler, err := NewScheduler(cfg, control, nil)
...
func (p *Processor) Submit(ctx context.Context, job OctaCoreJob) OctaCoreResult { return p.scheduler.Execute(ctx, job) }
func (p *Processor) Batch(ctx context.Context, jobs []OctaCoreJob) []OctaCoreResult { return p.scheduler.ExecutePlan(ctx, jobs) }
