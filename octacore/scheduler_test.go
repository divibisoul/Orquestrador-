...
func TestOctaCoreInventoryHasEightSlots(t *testing.T) {
    s := testScheduler(t)
    slots := s.Inventory()
    if len(slots) != 8 { t.Fatalf("expected 8 slots, got %d", len(slots)) }
    if slots[0].Slot != G0 || slots[7].Slot != G7 { t.Fatalf("unexpected slot boundaries: %#v", slots) }
}
...
func TestOctaCoreParallelGroupRunsConcurrently(t *testing.T) {
    ...
    if err := s.RegisterLocalKernel(G7, func(ctx context.Context, job OctaCoreJob) (map[string]any, error) {
        start := time.Now()
        mu.Lock(); starts[job.JobID] = start; mu.Unlock()
        select { case <-ctx.Done(): return nil, ctx.Err(); case <-time.After(80 * time.Millisecond): }
        mu.Lock(); ends[job.JobID] = time.Now(); mu.Unlock()
    ...
    for _, result := range results { if !result.OK { t.Fatalf("job failed: %#v", result.Error) } }
    if elapsed >= 140*time.Millisecond { t.Fatalf("parallel group appears serial: elapsed=%v", elapsed) }
    mu.Lock(); defer mu.Unlock()
    if starts["j1"].IsZero() || starts["j2"].IsZero() { t.Fatal("missing start timestamps") }
    if starts["j1"].After(ends["j2"]) || starts["j2"].After(ends["j1"]) { t.Fatal("jobs did not overlap") }
}
...
func TestOctaCoreBarrierWaitsForPriorGroup(t *testing.T) {
    ...
    if err := s.RegisterLocalKernel(G7, func(ctx context.Context, job OctaCoreJob) (map[string]any, error) {
        if job.JobID == "consumer" { mu.Lock(); consumerStart = time.Now(); mu.Unlock(); return map[string]any{"joined": true}, nil }
        select { case <-ctx.Done(): return nil, ctx.Err(); case <-time.After(60 * time.Millisecond): }
        mu.Lock(); ends[job.JobID] = time.Now(); mu.Unlock()
        return map[string]any{"done": true}, nil
    }); err != nil { t.Fatal(err) }
    group := "pre"; barrier := "pre"; consumerGroup := "post"
    results := s.ExecutePlan(context.Background(), []OctaCoreJob{
    ...
    if ends["p2"].After(latestProducer) { latestProducer = ends["p2"] }
    if consumerStart.Before(latestProducer) { t.Fatalf("barrier released before producer completion: consumer=%v producer=%v", consumerStart, latestProducer) }
}

func TestOctaCoreThrottleReducesInflight(t *testing.T) {
    s := testScheduler(t)
    if err := s.SetThrottle(3); err != nil { t.Fatal(err) }
    if err := s.RegisterLocalKernel(G7, func(ctx context.Context, job OctaCoreJob) (map[string]any, error) {
        select { case <-ctx.Done(): return nil, ctx.Err(); case <-time.After(50 * time.Millisecond): return map[string]any{"ok": true}, nil }
    }); err != nil { t.Fatal(err) }
    group := "throttle"
    started := time.Now()
    ...
    elapsed := time.Since(started)
    for _, result := range results { if !result.OK { t.Fatalf("job failed: %#v", result.Error) } }
    if elapsed < 90*time.Millisecond { t.Fatalf("throttle did not reduce concurrency: elapsed=%v", elapsed) }
    if results[1].Metrics.QueueWaitMS == 0 { t.Fatalf("expected queue wait under throttle, got %d", results[1].Metrics.QueueWaitMS) }
}

func TestOctaCoreCircuitBreakerOpensAfterFailures(t *testing.T) {
    cfg := DefaultSchedulerConfig(); cfg.FailureThreshold = 2; cfg.TokenRefillPerSec = 1000
    s, err := NewScheduler(cfg, nil, nil); if err != nil { t.Fatal(err) }
    if err := s.RegisterLocalKernel(G7, func(context.Context, OctaCoreJob) (map[string]any, error) { return nil, errors.New("kernel_failure") }); err != nil { t.Fatal(err) }
