...
func HTTPHandler(processor *Processor) http.Handler {
	...
	mux.HandleFunc("/v1/octacore/health", func(w http.ResponseWriter, r *http.Request) {
		if r.Method != http.MethodGet {
			writeOctaJSON(w, http.StatusMethodNotAllowed, map[string]any{"error": "GET required"})
			return
		}
		if err := authorizeOctaHTTP(r); err != nil {
			writeOctaJSON(w, http.StatusUnauthorized, map[string]any{"error": err.Error()})
			return
		}
	...
	mux.HandleFunc("/v1/octacore/submit", func(w http.ResponseWriter, r *http.Request) {
		if r.Method != http.MethodPost {
			writeOctaJSON(w, http.StatusMethodNotAllowed, map[string]any{"error": "POST required"})
			return
		}
		if err := authorizeOctaHTTP(r); err != nil {
			writeOctaJSON(w, http.StatusUnauthorized, map[string]any{"error": err.Error()})
		...
		if err := decoder.Decode(&job); err != nil {
			writeOctaJSON(w, http.StatusBadRequest, map[string]any{"error": "INVALID_OCTACORE_JSON", "detail": err.Error()})
			return
		}
		if header := strings.TrimSpace(r.Header.Get("X-Correlation-ID")); header != "" && header != job.CorrelationID {
			writeOctaJSON(w, http.StatusBadRequest, map[string]any{"error": "CORRELATION_MISMATCH"})
			return
		}
		if err := job.Validate(); err != nil {
			writeOctaJSON(w, http.StatusBadRequest, map[string]any{"error": "INVALID_OCTACORE_JOB", "detail": err.Error()})
			return
		}
	...
	mux.HandleFunc("/v1/octacore/batch", func(w http.ResponseWriter, r *http.Request) {
		...
		if err := decoder.Decode(&jobs); err != nil {
			writeOctaJSON(w, http.StatusBadRequest, map[string]any{"error": "INVALID_OCTACORE_BATCH_JSON", "detail": err.Error()})
			return
		}
		if len(jobs) == 0 {
			writeOctaJSON(w, http.StatusBadRequest, map[string]any{"error": "OCTACORE_BATCH_EMPTY"})
			return
		}
		for i, job := range jobs {
			if err := job.Validate(); err != nil {
				writeOctaJSON(w, http.StatusBadRequest, map[string]any{"error": "INVALID_OCTACORE_JOB", "index": i, "detail": err.Error()})
				return
			}
		}
