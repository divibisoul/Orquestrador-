...
func (b *tokenBucket) take(ctx context.Context) error {
    for {
        ...
        if elapsed > 0 { b.tokens = minFloat(b.capacity, b.tokens+elapsed*b.rate); b.last = now }
        if b.tokens >= 1 { b.tokens--; b.mu.Unlock(); return nil }
...
func (s *OctaCoreScheduler) SetThrottle(level int) error {
    if level < 0 || level > 3 { return errors.New("throttle level must be 0..3") }
    s.throttle.Store(int32(level)); return nil
}
...
func (s *OctaCoreScheduler) Execute(ctx context.Context, job OctaCoreJob) OctaCoreResult {
    ...
    if err := job.Validate(); err != nil { return failedResult(job, "INVALID_JOB", err, 0, 0) }
    if s.halted.Load() { return failedResult(job, "OCTACORE_HALTED", errors.New("Octacore scheduler halted"), 0, 0) }
    slot, err := s.resolveSlot(job)
    if err != nil { return failedResult(job, "SLOT_RESOLUTION_FAILED", err, 0, 0) }
    if slot.Slot != G0 && len(job.BackendPrefs) == 0 { job.BackendPrefs = []Backend{BackendRemoteMesh} }
    deadline := time.Now().Add(time.Duration(job.TTLMS) * time.Millisecond)
    if existing, ok := ctx.Deadline(); !ok || deadline.Before(existing) { var cancel context.CancelFunc; ctx, cancel = context.WithDeadline(ctx, deadline); defer cancel() }
    s.queue.Add(1); defer s.queue.Add(-1)
    if err := s.bucket.take(ctx); err != nil { return failedResult(job, "TOKEN_BUCKET_EXHAUSTED", err, 0, time.Since(queuedAt).Milliseconds()) }
    if err := s.acquireInflight(ctx, slot.Slot); err != nil { return failedResult(job, "THROTTLED", err, 0, time.Since(queuedAt).Milliseconds()) }
    ...
    if execErr != nil { result.Error = toOctaError(execErr); s.recordFailure(slot.Slot, latency, execErr.Error()) } else { s.recordSuccess(slot.Slot, latency) }
    source := string(slot.Slot); if slot.Slot == G0 { source = "SARA" }
...
func barrierReady(index int, job OctaCoreJob, pending map[int]OctaCoreJob) bool {
    name := strings.TrimSpace(ptrString(job.Barrier)); if name == "" { return true }
    group := strings.TrimSpace(ptrString(job.ParallelGroup))
    for i, other := range pending { if i >= index { continue }; if strings.TrimSpace(ptrString(other.Barrier)) != name { continue }; otherGroup := strings.TrimSpace(ptrString(other.ParallelGroup)); if group != "" && group == otherGroup { continue }; return false }
    return true
}

func (s *OctaCoreScheduler) resolveSlot(job OctaCoreJob) (OctaCoreSlot, error) {
    if job.Kind == KindSARAudit || job.Kind == KindSARACycle { return s.slots[G0], nil }
    if job.Target != "scheduler" { slot := s.slots[SlotID(job.Target)]; if slot.Slot == "" { return OctaCoreSlot{}, fmt.Errorf("unknown slot: %s", job.Target) }; return slot, nil }
    switch job.Kind { case KindResearch, KindTool: return s.slots[G4], nil; case KindPerceive: return s.slots[G3], nil; case KindSessionStep: return s.slots[G6], nil; case KindDispatch: return s.slots[G7], nil; case KindCustom: return OctaCoreSlot{}, errors.New("custom job requires explicit target"); default: return OctaCoreSlot{}, fmt.Errorf("no scheduler route for kind: %s", job.Kind) }
}
...
func (s *OctaCoreScheduler) executeG0(ctx context.Context, job OctaCoreJob) (map[string]any, string, error) {
    if s.sara == nil || !s.sara.Configured() { return nil, string(BackendSARAHTTP), errors.New("SARA_UNAVAILABLE:SARA service is not configured") }
    input, _ := job.Payload["input"].(string); if strings.TrimSpace(input) == "" { return nil, string(BackendSARAHTTP), errors.New("SARA_INPUT_REQUIRED") }
    switch job.Kind { case KindSARAudit: out, err := s.sara.Audit(ctx, input, job.CorrelationID); return out, string(BackendSARAHTTP), err; case KindSARACycle: cycleID, _ := job.Payload["cycle_id"].(string); out, err := s.sara.Cycle(ctx, input, cycleID, job.CorrelationID); return out, string(BackendSARAHTTP), err }
    operation, _ := job.Payload["operation"].(string); switch strings.TrimSpace(operation) { case "sara.audit": out, err := s.sara.Audit(ctx, input, job.CorrelationID); return out, string(BackendSARAHTTP), err; case "sara.cycle": cycleID, _ := job.Payload["cycle_id"].(string); out, err := s.sara.Cycle(ctx, input, cycleID, job.CorrelationID); return out, string(BackendSARAHTTP), err; case "sara.regenerate": out, err := s.sara.Regenerate(ctx, input, job.CorrelationID); return out, string(BackendSARAHTTP), err; case "sara.state": out, err := s.sara.State(ctx, job.CorrelationID); return out, string(BackendSARAHTTP), err; case "sara.trace": cycleID, _ := job.Payload["cycle_id"].(string); out, err := s.sara.Trace(ctx, cycleID, job.CorrelationID); return out, string(BackendSARAHTTP), err; default: return nil, string(BackendSARAHTTP), fmt.Errorf("SARA_OPERATION_UNSUPPORTED:%s", strings.TrimSpace(operation)) }
}

func (s *OctaCoreScheduler) executeRemote(ctx context.Context, job OctaCoreJob, slot OctaCoreSlot) (map[string]any, string, error) {
    capability, _ := job.Payload["capability"].(string); capability = strings.TrimSpace(capability); if capability == "" { return nil, string(BackendRemoteMesh), errors.New("REMOTE_CAPABILITY_REQUIRED") }
    payload := cloneMap(job.Payload); delete(payload, "capability"); payload["octacore"] = map[string]any{"job_id": job.JobID, "correlation_id": job.CorrelationID, "source": job.Source, "target": slot.Slot, "kind": job.Kind}
    callCapability := capability
    if slot.Slot == G4 || slot.Slot == G6 { callCapability = "octacore.execute"; payload = map[string]any{"capability": capability, "payload": cloneMap(job.Payload)} }
    out, err := s.peers.CallWithCorrelation(ctx, slot.Nucleus, callCapability, payload, job.CorrelationID); return out, string(BackendRemoteMesh), err
}
...
func (s *OctaCoreScheduler) acquireInflight(ctx context.Context, slot SlotID) error {
    for { if err := ctx.Err(); err != nil { return err }; if !s.slotCircuitAllows(slot) { return fmt.Errorf("slot circuit open: %s", slot) }; limit := s.effectiveInflightLimit(); if int(s.inflight.Load()) < limit { s.inflight.Add(1); st := s.state[slot]; st.mu.Lock(); st.inflight++; st.mu.Unlock(); return nil }; timer := time.NewTimer(5*time.Millisecond); select { case <-ctx.Done(): timer.Stop(); return ctx.Err(); case <-timer.C: } }
}
func (s *OctaCoreScheduler) releaseInflight(slot SlotID) { s.inflight.Add(-1); st := s.state[slot]; st.mu.Lock(); st.inflight--; if st.halfOpenInUse { st.halfOpenInUse = false }; st.mu.Unlock() }
func (s *OctaCoreScheduler) effectiveInflightLimit() int { base := s.cfg.MaxInflight; switch int(s.throttle.Load()) { case 1: base=maxInt(1,base/2); case 2: base=maxInt(1,base/4); case 3: base=1 }; return base }
func (s *OctaCoreScheduler) slotCircuitAllows(slot SlotID) bool { st:=s.state[slot]; st.mu.Lock(); defer st.mu.Unlock(); if st.circuit != string(CircuitOpen) { return true }; if time.Now().Before(st.openUntil) { return false }; if st.halfOpenInUse { return false }; st.circuit=string(CircuitHalfOpen); st.halfOpenInUse=true; return true }
func (s *OctaCoreScheduler) recordSuccess(slot SlotID, latencyMS int64) { st:=s.state[slot]; st.mu.Lock(); st.failures=0; st.circuit=string(CircuitClosed); st.openUntil=time.Time{}; st.lastLatency=time.Duration(latencyMS)*time.Millisecond; st.lastError=""; st.mu.Unlock() }
...
func newVagusEvent(typ, source, target string, priority int, ttl int64, correlation string, payload map[string]any) VagusEnvelope { return VagusEnvelope{VagusVersion:VagusVersion,MessageID:NewJobID(),CorrelationID:correlation,Source:source,Target:target,Priority:priority,TTL:ttl,Type:typ,Payload:payload} }
func failedResult(job OctaCoreJob, code string, err error, latencyMS, queueWaitMS int64) OctaCoreResult { if err==nil {err=errors.New(code)}; return OctaCoreResult{JobID:job.JobID,CorrelationID:job.CorrelationID,OK:false,Error:&OctaCoreError{Code:code,Message:err.Error()},Metrics:OctaCoreMetrics{LatencyMS:latencyMS,QueueWaitMS:queueWaitMS}} }
func toOctaError(err error) *OctaCoreError { if err==nil{return nil}; msg:=err.Error(); code:="EXECUTION_ERROR"; if strings.Contains(msg, "SARA_"){code=stringBefore(msg, ":")}; return &OctaCoreError{Code:code,Message:msg} }
func stringBefore(value, sep string) string { if i:=strings.Index(value,sep); i>0 {return value[:i]}; return value }
func parseNestedJob(payload map[string]any) (OctaCoreJob,bool){raw,ok:=payload["job"];if !ok{return OctaCoreJob{},false};b,err:=json.Marshal(raw);if err!=nil{return OctaCoreJob{},false};var job OctaCoreJob;if err=json.Unmarshal(b,&job);err!=nil{return OctaCoreJob{},false};return job,job.Validate()==nil}
func numberArray(value any)([]float64,error){switch v:=value.(type){case []float64:return append([]float64(nil),v...),nil;case []any:out:=make([]float64,len(v));for i,item:=range v{n,ok:=item.(float64);if !ok{if str,yes:=item.(string);yes{parsed,err:=strconv.ParseFloat(str,64);if err!=nil{return nil,fmt.Errorf("invalid numeric value at index %d",i)};n=parsed}else{return nil,fmt.Errorf("invalid numeric value at index %d",i)}};out[i]=n};return out,nil;default:return nil,errors.New("values must be numeric array")}}
func cloneMap(in map[string]any)map[string]any{out:=make(map[string]any,len(in));for k,v:=range in{out[k]=v};return out}
func ptrString(v *string)string{if v==nil{return ""};return *v}
func barrierCorrelation(jobs []OctaCoreJob,name string)string{for _,job:=range jobs{if strings.TrimSpace(ptrString(job.Barrier))==name&&strings.TrimSpace(job.CorrelationID)!=""{return job.CorrelationID}};return NewJobID()}
