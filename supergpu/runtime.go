...
func (CPUBackend) Execute(ctx context.Context, _ Device, op string, in []float64) ([]float64, error) {
	...
	for _, v := range in {
		if math.IsNaN(v) || math.IsInf(v, 0) {
			return nil, errors.New("workload contains non-finite value")
		}
	}
	switch op {
	case "identity":
...
func (r *Runtime) Discover() []Device {
	...
	if time.Since(r.discoverAt) < r.discoveryTTL && len(r.devices) > 0 {
	...
	if _, err := exec.LookPath("nvidia-smi"); err == nil {
		devices = append(devices, Device{ID: "nvidia-auto", Vendor: "NVIDIA", Name: "NVIDIA GPU detected; compatible backend required", Available: false, Backend: "nvidia"})
	}
	if _, err := exec.LookPath("rocminfo"); err == nil {
		devices = append(devices, Device{ID: "amd-auto", Vendor: "AMD", Name: "AMD GPU detected; compatible backend required", Available: false, Backend: "rocm"})
	}
	if cb, ok := r.backend.(CapabilityBackend); ok {
		for i, d := range devices {
			if d.Backend != "cpu" {
				devices[i].Available = cb.Supports(d)
				devices[i].Capabilities = append([]string(nil), cb.Capabilities(d)...)
			}
		}
	}
	r.devices = devices
	r.discoverAt = time.Now()
	return append([]Device(nil), r.devices...)
}
func (r *Runtime) Select(preferred string) (Device, error) {
	...
	defer r.mu.Unlock()
	if r.closed {
		return Device{}, errors.New("runtime closed")
	}
	r.expireReservationsLocked()
	if len(r.devices) == 0 {
		return Device{}, errors.New("no devices discovered")
	}
	if preferred != "" {
		for _, d := range r.devices {
			if d.ID == preferred {
				if !d.Available {
					return Device{}, errors.New("preferred device unavailable")
				}
				if _, busy := r.reserved[d.ID]; busy {
					return Device{}, errors.New("preferred device reserved")
				}
				return d, nil
			}
		}
		return Device{}, errors.New("preferred device not found")
	}
	for _, d := range r.devices {
		if d.Available {
			if _, busy := r.reserved[d.ID]; !busy {
...
func (r *Runtime) Reserve(deviceID, owner string) error {
	if deviceID == "" || owner == "" {
		return errors.New("device and owner are required")
	}
	r.mu.Lock()
	defer r.mu.Unlock()
	if r.closed {
	...
	r.reserved[deviceID] = Reservation{DeviceID: deviceID, Owner: owner, ExpiresAt: time.Now().Add(30 * time.Second), Priority: 0}
	return nil
}
func (r *Runtime) Release(deviceID, owner string) error {
	r.mu.Lock()
	defer r.mu.Unlock()
	current, ok := r.reserved[deviceID]
	if !ok {
		return nil
	}
...
func (r *Runtime) Batch(ctx context.Context, device Device, operation string, inputs [][]float64) ([][]float64, error) {
	...
	for i, in := range inputs {
		...
		v, err := r.Execute(ctx, device, operation, in)
		if err != nil {
			return nil, err
		}
		out[i] = v
...
// BatchParallel executes independent workloads with bounded concurrency, preserving input order.
func (r *Runtime) BatchParallel(ctx context.Context, device Device, operation string, inputs [][]float64, workers int) ([][]float64, error) {
	if ctx == nil {
		return nil, errors.New("context is nil")
	}
	if len(inputs) == 0 {
...
func (r *Runtime) MemoryStats(device Device) map[string]any {
	r.mu.RLock()
	defer r.mu.RUnlock()
	return map[string]any{"device_id": device.ID, "memory_bytes": device.MemoryBytes, "memory_usage_supported": false, "backend": device.Backend, "available": device.Available, "capabilities": append([]string(nil), device.Capabilities...)}
}
func (r *Runtime) Health() map[string]any {
	...
	return map[string]any{"status": status, "devices": len(r.devices), "reservations": len(r.reserved), "backend": true, "discovery_age_ms": time.Since(r.discoverAt).Milliseconds()}
}
...
func (r *Runtime) expireReservationsLocked() {
	now := time.Now()
	for id, res := range r.reserved {
