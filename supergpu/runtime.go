...
type CapabilityBackend interface{Backend; Supports(Device) bool}
...
func(CPUBackend)Execute(ctx context.Context,_ Device,op string,in []float64)([]float64,error){if ctx==nil{return nil,errors.New("context is nil")};if len(in)==0{return nil,errors.New("empty workload")};out:=make([]float64,len(in));switch op{case "identity":copy(out,in);case "relu":for i,v:=range in{if v>0{out[i]=v}};case "square":for i,v:=range in{out[i]=v*v};case "normalize":var s float64;for _,v:=range in{s+=v*v};norm:=math.Sqrt(s+1e-12);for i,v:=range in{out[i]=v/norm};default:return nil,errors.New("unsupported compute operation")};select{case<-ctx.Done():return nil,ctx.Err();default:return out,nil}}
...
func(r *Runtime)Discover()[]Device{r.mu.Lock();defer r.mu.Unlock();if r.closed{return nil};devices:=[]Device{{ID:"cpu-0",Vendor:runtime.GOOS,Name:"host-compute",Available:true,Backend:"cpu"}};if _,err:=exec.LookPath("nvidia-smi");err==nil{devices=append(devices,Device{ID:"nvidia-auto",Vendor:"NVIDIA",Name:"NVIDIA GPU detected; native backend required",Available:false,Backend:"nvidia"})};if _,err:=exec.LookPath("rocminfo");err==nil{devices=append(devices,Device{ID:"amd-auto",Vendor:"AMD",Name:"AMD GPU detected; native backend required",Available:false,Backend:"rocm"})};if cb,ok:=r.backend.(CapabilityBackend);ok{for i,d:=range devices{if d.Backend!="cpu"{devices[i].Available=cb.Supports(d)}}};r.devices=devices;return append([]Device(nil),devices...)}
...
func(r *Runtime)Reserve(deviceID,owner string)error{if deviceID==""||owner==""{return errors.New("device and owner are required")};r.mu.Lock();defer r.mu.Unlock();if r.closed{return errors.New("runtime closed")};if current,ok:=r.reserved[deviceID];ok&&current!=owner{return errors.New("device already reserved")};var available bool;for _,d:=range r.devices{if d.ID==deviceID{available=d.Available;break}};if !available{return errors.New("device unavailable")};r.reserved[deviceID]=owner;return nil}
