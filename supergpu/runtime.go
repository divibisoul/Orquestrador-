...
func (r *Runtime) MemoryStats(device Device) map[string]any {
	r.mu.RLock()
	defer r.mu.RUnlock()
	return map[string]any{"device_id": device.ID, "memory_bytes": device.MemoryBytes, "memory_usage_supported": device.MemoryBytes > 0, "backend": device.Backend, "available": device.Available, "accelerator": device.Backend != "cpu", "capabilities": append([]string(nil), device.Capabilities...)}
}
