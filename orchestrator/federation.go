...
func NewFederation() *Federation {
	return &Federation{peers: map[string]*peerState{}, failureThreshold: 3, breakerWindow: 5 * time.Second, maxRetries: 2, baseBackoff: 50 * time.Millisecond}
}
func (f *Federation) RegisterPeer(nucleus, baseURL string, capabilities []string) error {
	...
	f.mu.Lock()
	f.peers[nucleus] = &peerState{Peer: Peer{Nucleus: nucleus, URL: baseURL, Capabilities: uniqueStrings(append([]string(nil), capabilities...)), Healthy: true, SuccessRate: 1, Adapter: adapter}}
	f.mu.Unlock()
	return nil
}
...
func (f *Federation) Stats() map[string]any {
	...
	for _, s := range f.peers {
		...
		peers = append(peers, map[string]any{"nucleus": s.Nucleus, "healthy": s.Healthy, "latency_ms": float64(s.Latency) / float64(time.Millisecond), "success_rate": s.SuccessRate, "in_flight": s.InFlight, "calls": s.calls, "success": s.success, "circuit_open": now.Before(s.openUntil)})
	}
	return map[string]any{"peers": len(f.peers), "healthy": healthy, "calls": calls, "success": success, "success_rate": func() float64 {
		if calls == 0 {
			return 0
		}
		return float64(success) / float64(calls)
	}(), "circuit_open": open, "details": peers}
}
...
func (f *Federation) Discover(capability string) []Candidate {
	...
	for _, state := range f.peers {
		...
		candidates = append(candidates, Candidate{Nucleus: state.Nucleus, Capability: capability, Score: score, Latency: latency, SuccessRate: success, Healthy: state.Healthy, InFlight: state.InFlight})
	}
	f.mu.RUnlock()
