...
func (e *Engine) Register(operation string, handler Handler) error {
	if operation == "" || handler == nil {
		return errors.New("operation and handler are required")
	}
	name, version := operation, "1.0.0"
	if i := strings.LastIndex(operation, "@"); i > 0 {
		name, version = operation[:i], operation[i+1:]
	}
	if !semverRx.MatchString(version) {
		return errors.New("operation version must be semantic version")
	}
	e.mu.Lock()
	defer e.mu.Unlock()
	...
	e.handlers[name] = OperationRegistration{Name: name, Version: version, Handler: handler, Timeout: 15 * time.Second, RateLimit: e.rateLimit, Metadata: map[string]string{"capability": name}}
...
func (e *Engine) Operations() []string {
	...
	for name, reg := range e.handlers {
		if reg.Handler != nil {
			ops = append(ops, name+"@"+reg.Version)
		}
	}
	e.mu.RUnlock()
	sort.Strings(ops)
...
func (e *Engine) Route(m protocol.Message) (Handler, error) {
	...
	now := time.Now()
	e.mu.RLock()
	if r, ok := e.routes[m.Operation]; ok && now.Before(r.expires) {
		e.mu.RUnlock()
		return r.handler, nil
	}
	reg, ok := e.handlers[m.Operation]
	e.mu.RUnlock()
	if !ok {
	...
	e.mu.Lock()
	e.routes[m.Operation] = routeCacheEntry{handler: reg.Handler, expires: now.Add(e.routeTTL)}
	e.mu.Unlock()
...
func (e *Engine) Submit(ctx context.Context, m protocol.Message) (protocol.Result, error) {
	...
	if !e.running.Load() {
		...
		return protocol.Result{TraceID: m.TraceID, Source: "N07", Target: m.Source, Status: "rejected", Error: err.Error()}, err
	}
	if ctx == nil {
	...
	if e.breakerOpen() {
		...
		e.metrics.Observe(start, err, false)
		return protocol.Result{TraceID: m.TraceID, Source: "N07", Target: m.Source, Status: "busy", Error: err.Error()}, err
	}
	if err := m.Validate(); err != nil {
		e.metrics.Observe(start, err, false)
		return protocol.Result{TraceID: m.TraceID, Source: "N07", Target: m.Source, Status: "rejected", Error: err.Error()}, err
	...
	if !e.allow(m.Source) {
		...
		e.metrics.Observe(start, err, false)
		return protocol.Result{TraceID: m.TraceID, Source: "N07", Target: m.Source, Status: "rate_limited", Error: err.Error()}, err
	}
	h, err := e.Route(m)
	if err != nil {
		e.recordFailure()
	...
	e.logger.Info(m.Operation, m.TraceID, map[string]any{"payload_size": len(m.Payload), "operation_version": reg.Version})
	...
	if metadata != nil {
		if s := metadata["schema"]; s != "" {
			if err := validateSchema(s, payload); err != nil {
				return protocol.Result{TraceID: m.TraceID, Source: "N07", Target: "N07", Status: "rejected", Error: err.Error()}, err
			}
		}
	}
...
func (e *Engine) Health() map[string]any {
	...
	e.mu.RUnlock()
	return map[string]any{"nucleus": "N07", "status": e.Status(), "handlers": handlers, "active_traces": active, "metrics": e.metrics.Snapshot(), "circuit": map[string]any{"state": map[bool]string{true: "open", false: "closed"}[e.breakerOpen()]}, "neural": e.neural.Health(), "prefrontal": e.cortex.Health(), "compute": e.compute.Health()}
}
...
func (e *Engine) registerBuiltins() error {
	if err := e.Register("neural.forward@1.0.0", func(ctx context.Context, m protocol.Message) (protocol.Result, error) {
		e.mu.Lock()
		if t, ok := e.active[m.TraceID]; ok {
			t.stage = "neural"
			e.active[m.TraceID] = t
		}
		e.mu.Unlock()
		v, err := e.neural.Forward(ctx, m.Payload)
		return protocol.Result{TraceID: m.TraceID, Source: "N07.neural", Target: m.Source, Status: status(err), Payload: v, Error: errorText(err)}, err
	}); err != nil {
		return err
	}
	if err := e.Register("neural.learn@1.0.0", func(ctx context.Context, m protocol.Message) (protocol.Result, error) {
		...
		return protocol.Result{TraceID: m.TraceID, Source: "N07.neural", Target: m.Source, Status: status(err), Error: errorText(err)}, err
	}); err != nil {
		return err
	}
	if err := e.Register("compute.execute@1.0.0", func(ctx context.Context, m protocol.Message) (protocol.Result, error) {
		...
		return protocol.Result{TraceID: m.TraceID, Source: "N07.compute", Target: m.Source, Status: status(err), Payload: v, Error: errorText(err)}, err
	}); err != nil {
		return err
	}
	if err := e.Register("cognitive.execute@1.0.0", func(ctx context.Context, m protocol.Message) (protocol.Result, error) {
		...
		if err != nil {
			return protocol.Result{TraceID: m.TraceID, Source: "N07", Target: m.Source, Status: "error", Error: err.Error()}, err
		}
		energy := 0.0
		...
		if err != nil {
			return protocol.Result{TraceID: m.TraceID, Source: "N07.prefrontal", Target: m.Source, Status: "rejected", Error: err.Error()}, err
		}
		...
		d, err := e.compute.Select(m.Metadata["device"])
		if err != nil {
			return protocol.Result{TraceID: m.TraceID, Source: "N07.compute", Target: m.Source, Status: "error", Error: err.Error()}, err
		}
		if err = e.compute.Reserve(d.ID, m.TraceID); err != nil {
			return protocol.Result{TraceID: m.TraceID, Source: "N07.compute", Target: m.Source, Status: "busy", Error: err.Error()}, err
		}
		defer e.compute.Release(d.ID, m.TraceID)
		e.mu.Lock()
		if t, ok := e.active[m.TraceID]; ok {
		...
		return protocol.Result{TraceID: m.TraceID, Source: "N07.pipeline", Target: m.Source, Status: status(err), Payload: out, Metadata: map[string]string{"decision": "approved", "device": d.ID}, Error: errorText(err)}, err
	}); err != nil {
		return err
	}
	return nil
