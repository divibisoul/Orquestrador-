...
func (e *Engine) Route(m protocol.Message) (Handler, error) {
	...
	reg, ok := e.resolveRegistrationLocked(name, version)
	if !ok {
...
func (e *Engine) Submit(ctx context.Context, m protocol.Message) (protocol.Result, error) {
	...
	reg, err := e.registration(m.Operation)
	if err != nil {
		e.recordFailure()
		e.metrics.Observe(start, err, false)
		return protocol.Result{TraceID: m.TraceID, CorrelationID: m.CorrelationID, Source: "N07", Target: m.Source, Status: "rejected", Error: err.Error()}, err
	...
	if err != nil {
		e.recordFailure()
		e.logger.Error(m.Operation, m.TraceID, err, map[string]any{"cancelled": cancelled})
	} else {
		e.recordSuccess()
	}
	return result, err
...
func (e *Engine) registerBuiltins() error {
	...
	if err := e.Register("cognitive.execute@1.0.0", func(ctx context.Context, m protocol.Message) (protocol.Result, error) {
		...
		d, err := e.compute.Select(m.Metadata["device"])
		if err != nil {
			return protocol.Result{TraceID: m.TraceID, CorrelationID: m.CorrelationID, Source: "N07.compute", Target: m.Source, Status: "error", Error: err.Error()}, err
		}
		if err = e.compute.Reserve(d.ID, m.TraceID); err != nil {
			return protocol.Result{TraceID: m.TraceID, CorrelationID: m.CorrelationID, Source: "N07.compute", Target: m.Source, Status: "busy", Error: err.Error()}, err
		}
		defer e.compute.Release(d.ID, m.TraceID)
		e.setStage(m.TraceID, "compute", d.ID)
		op := m.Metadata["operation"]
...
func validateSchema(schema string, payload []float64) error {
	...
	if t, ok := raw["type"].(string); ok && t != "array" {
		return errors.New("payload schema type must be array")
	}
	if max, ok := raw["maxItems"].(float64); ok && float64(len(payload)) > max {
		return errors.New("payload exceeds schema maxItems")
	}
	if min, ok := raw["minItems"].(float64); ok && float64(len(payload)) < min {
