...
func splitOperation(operation string) (string,string,error) {
	operation=strings.TrimSpace(operation); if operation=="" { return "","",errors.New("operation is required") }
	name,version:=operation,""; if i:=strings.LastIndex(operation,"@"); i>0 { name,version=operation[:i],operation[i+1:] }
	if version!=""&&!semverRx.MatchString(version){return "","",errors.New("operation version must semantic version")}
...
func(e *Engine)resolveRegistrationLocked(name,version string)(OperationRegistration,bool){if version!=""{reg,ok:=e.handlers[name+"@"+version];return reg,ok};var reg OperationRegistration;var ok bool;for _,candidate:=range e.handlers{if candidate.Name!=name||candidate.Handler==nil{continue};if !ok||semverGreater(candidate.Version,reg.Version){reg,ok=candidate,true}};return reg,ok}
func(e *Engine)Route(m protocol.Message)(Handler,error){if err:=m.Validate();err!=nil{return nil,err};name,version,err:=splitOperation(m.Operation);if err!=nil{return nil,err};now:=time.Now();cacheKey:=name+"@"+version;e.mu.RLock();if r,ok:=e.routes[cacheKey];ok&&now.Before(r.expires){e.mu.RUnlock();return r.handler,nil};e.mu.RUnlock();e.mu.Lock();defer e.mu.Unlock();if r,ok:=e.routes[cacheKey];ok&&now.Before(r.expires){return r.handler,nil};reg,ok:=e.resolveRegistrationLocked(name,version);if !ok{return nil,errors.New("no route for operation: "+m.Operation)};e.routes[cacheKey]=routeCacheEntry{handler:reg.Handler,expires:now.Add(e.routeTTL),operation:reg.Name,version:reg.Version};return reg.Handler,nil}
func(e *Engine)registration(operation string)(OperationRegistration,error){name,version,err:=splitOperation(operation);if err!=nil{return OperationRegistration{},err};e.mu.RLock();reg,ok:=e.resolveRegistrationLocked(name,version);e.mu.RUnlock();if !ok||reg.Handler==nil{return OperationRegistration{},errors.New("registered operation unavailable")};return reg,nil}
