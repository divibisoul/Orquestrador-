...
func RegisterSuperGPUOperations(e *Engine) error {
	...
	registrations := []struct {
	...
	}{
		{
			...
			handler: func(ctx context.Context, message protocol.Message) (protocol.Result, error) {
				select { case <-ctx.Done(): return protocol.Result{}, ctx.Err(); default: }
		...
		},
		{
			...
			handler: func(ctx context.Context, message protocol.Message) (protocol.Result, error) {
				...
				if err != nil { return protocol.Result{}, err }
				if err := e.compute.Reserve(device.ID, message.TraceID); err != nil { return protocol.Result{}, err }
		...
		},
		{
			...
			handler: func(ctx context.Context, message protocol.Message) (protocol.Result, error) {
				...
				defer e.compute.Release(device.ID, message.TraceID)
				var inputs [][]float64
				if err := json.Unmarshal([]byte(message.Metadata["inputs_json"]), &inputs); err != nil || len(inputs) == 0 { return protocol.Result{}, errors.New("metadata.inputs_json must contain a non-empty numeric batch") }
				op := strings.TrimSpace(message.Metadata["operation"])
				if op == "" { op = "identity" }
				workers := 1
				if raw := strings.TrimSpace(message.Metadata["workers"]); raw != "" {
					if parsed, parseErr := strconv.Atoi(raw); parseErr == nil && parsed > 0 { workers = parsed }
				}
				values, err := e.compute.BatchParallel(ctx, device, op, inputs, workers)
				encoded, encodeErr := json.Marshal(values)
				if err != nil { return protocol.Result{TraceID: message.TraceID, CorrelationID: message.CorrelationID, Source: "N07.supergpu", Target: message.Source, Status: status(err), Metadata: map[string]string{"device": device.ID, "operation": op}}, err }
				if encodeErr != nil { return protocol.Result{}, encodeErr }
	...
	for _, item := range registrations {
		if err := e.Register(item.name, item.handler); err != nil {
			if strings.Contains(err.Error(), "already registered") { continue }
			return err
		}
	}
	return nil
