...
func TestSubmitMeshPropagatesExecutionError(t *testing.T){n,_:=neural.New(1,.1);c,_:=prefrontal.New(.1,2);e,_:=New(n,c,supergpu.New(nil));env:=protocol.MeshEnvelope{ContractVersion:protocol.MeshContractVersion,Operation:"missing.operation",CorrelationID:"trace-error-1",Source:"N01",Target:"N07"};out,err:=e.SubmitMesh(context.Background(),env);if err==nil{t.Fatal("execution error swallowed")};if out.CorrelationID!="trace-error-1"||out.Source!="N07"||out.Target!="N01"{t.Fatalf("error response correlation lost: %+v",out)}}
