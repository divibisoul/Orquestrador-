...
func New(size int, learningRate float64) (*Network, error) {
	...
	return &Network{size: size, edges: make(map[int][]Edge), bias: make([]float64, size), learningRate: learningRate, config: Config{Layers: []Layer{{Activation: "tanh"}}, Optimizer: "adam", Regularization: 1e-6, GradientClip: 1.0, Heads: 1, BatchCache: 128}, adamM: make([]float64, size), adamV: make([]float64, size), cache: make(map[string][]float64)}, nil
}
func (n *Network) AddEdge(from, to int, weight float64) error {
	...
	n.edges[from] = append(n.edges[from], Edge{From: from, To: to, Weight: weight, Name: "edge-" + time.Now().UTC().Format("150405.000000000")})
	return nil
}
...
func (n *Network) Activate(inputs []float64) ([]float64, error) {
	...
	layerActivation := n.config.Layers[0].Activation
	n.mu.RUnlock()
	...
	for off := 0; off < len(inputs); off += n.size {
		for i := 0; i < n.size; i++ {
			...
			out[off+i] = activateValue(v+bias[i], layerActivation)
		}
	}
	n.mu.Lock()
...
func (n *Network) Forward(ctx context.Context, inputs []float64) ([]float64, error) {
	...
	for pass := 0; pass < len(cfg.Layers); pass++ {
		...
		for i := range next {
			...
			if cfg.Layers[pass].DropoutRate > 0 {
				next[i] *= (1 - cfg.Layers[pass].DropoutRate)
			}
		}
		state = next
	}
...
func (n *Network) Learn(inputs, target []float64) error {
	...
	for i := range pred {
		pred[i] = math.Tanh(inputs[i] + n.bias[i])
	}
	...
	sumAbs := 0.0
	for i := range pred {
		grad := (pred[i]-target[i])*(1-pred[i]*pred[i]) + n.config.Regularization*n.bias[i]
		if grad > clip {
			grad = clip
		}
		if grad < -clip {
		...
		case "sgd":
			n.bias[i] -= n.learningRate * grad
		case "rmsprop":
			n.adamV[i] = .9*n.adamV[i] + .1*grad*grad
			n.bias[i] -= n.learningRate * grad / (math.Sqrt(n.adamV[i]) + 1e-8)
		default:
			t := float64(n.stats.LearningSteps + 1)
			n.adamM[i] = .9*n.adamM[i] + .1*grad
			n.adamV[i] = .999*n.adamV[i] + .001*grad*grad
			mh := n.adamM[i] / (1 - math.Pow(.9, t))
			vh := n.adamV[i] / (1 - math.Pow(.999, t))
			n.bias[i] -= n.learningRate * mh / (math.Sqrt(vh) + 1e-8)
		}
		sumAbs += math.Abs(grad)
	}
	n.stats.LearningSteps++
...
func (n *Network) Backprop(inputs, target []float64) ([]float64, error) {
	...
	for i := range inputs {
		...
		pred := math.Tanh(inputs[i])
		grad[i] = (pred - target[i]) * (1 - pred*pred)
		if grad[i] > clip {
			grad[i] = clip
		}
		if grad[i] < -clip {
...
func (n *Network) Health() map[string]any {
	...
	return map[string]any{"status": "ready", "size": n.size, "edges": edges, "density": density, "learning_steps": n.stats.LearningSteps, "last_update": n.stats.LastUpdate, "last_gradient": n.stats.LastGradient, "activations": n.stats.Activations, "optimizer": n.config.Optimizer, "heads": n.config.Heads}
}
