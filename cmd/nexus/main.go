...
func main() {
	syncMeshSecretAlias()
	n, err := neural.New(8, .05); if err != nil { log.Fatal(err) }
	c, err := prefrontal.New(.10, 32); if err != nil { log.Fatal(err) }
	...
	mux.HandleFunc("/status", func(w http.ResponseWriter, r *http.Request) { if err := requireAppBearer(r); err != nil { writeJSON(w,http.StatusUnauthorized,map[string]string{"error":err.Error()});return };writeJSON(w,http.StatusOK,e.Stats()) })
...
func syncMeshSecretAlias(){primary:=strings.TrimSpace(os.Getenv("SOUL_MESH_HMAC_SECRET"));legacy:=strings.TrimSpace(os.Getenv("SOUL_MESH_SECRET"));if primary==""&&legacy!=""{if err:=os.Setenv("SOUL_MESH_HMAC_SECRET",legacy);err!=nil{log.Printf("mesh secret alias setup failed: %v",err)}}}
func requireAppBearer(r *http.Request)error{expected:=strings.TrimSpace(os.Getenv("N07_APP_TOKEN"));if expected==""{return errors.New("N07_APP_TOKEN is not configured")};authorization:=strings.TrimSpace(r.Header.Get("Authorization"));if len(authorization)<7||!strings.EqualFold(authorization[:7],"bearer "){return errors.New("Bearer authentication required")};provided:=strings.TrimSpace(authorization[7:]);if provided==""||provided!=expected{return errors.New("invalid application token")};return nil}
func writeJSON(w http.ResponseWriter,status int,v any){w.Header().Set("Content-Type","application/json");w.WriteHeader(status);_ = json.NewEncoder(w).Encode(v)}
func writeMetrics(w http.ResponseWriter,s map[string]any){w.Header().Set("Content-Type","text/plain; version=0.0.4");m,ok:=s["metrics"].(map[string]any);if !ok{w.WriteHeader(http.StatusInternalServerError);_,_ = w.Write([]byte("n07_metrics_error 1\n"));return};for _,k:=range []string{"requests","success","errors","cancelled","in_flight","latency_p95_ms"}{if v,exists:=m[k];exists{_,_=fmt.Fprintf(w,"n07_%s %v\n",k,v)}}}
