...
func main() {
	...
	mux.HandleFunc("/execute", func(w http.ResponseWriter, r *http.Request) {
		...
		if err != nil { writeJSON(w, http.StatusBadRequest, result); return }
	...
	if registry := strings.TrimSpace(os.Getenv("N07_MESH_REGISTRY_URL")); registry != "" {
		go func() {
			...
			if err != nil { log.Printf("mesh adapter init failed: %v", err); return }
	...
	go func() {
		...
		if err := srv.ListenAndServe(); err != nil && !errors.Is(err, http.ErrServerClosed) { log.Printf("server error: %v", err) }
	}()
	<-ctx.Done()
...
func syncMeshSecretAlias() {
	...
	if primary == "" && legacy != "" {
		if err := os.Setenv("SOUL_MESH_HMAC_SECRET", legacy); err != nil { log.Printf("mesh secret alias setup failed: %v", err) }
	}
}
func requireAppBearer(r *http.Request) error {
	...
	if len(authorization) < 7 || !strings.EqualFold(authorization[:7], "bearer ") { return errors.New("Bearer authentication required") }
	provided := strings.TrimSpace(authorization[7:])
	if provided == "" || provided != expected { return errors.New("invalid application token") }
	return nil
}
...
func writeMetrics(w http.ResponseWriter, s map[string]any) {
	...
	if !ok { w.WriteHeader(http.StatusInternalServerError); _, _ = w.Write([]byte("n07_metrics_error 1\n")); return }
	for _, k := range []string{"requests", "success", "errors", "cancelled", "in_flight", "latency_p95_ms"} { if v, exists := m[k]; exists { _, _ = fmt.Fprintf(w, "n07_%s %v\n", k, v) } }
}
