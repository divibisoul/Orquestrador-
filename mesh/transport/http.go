...
func (c Client) Do(ctx context.Context, endpoint string, env Envelope) (Envelope, error) {
    ...
    if endpoint == "" { return Envelope{}, errors.New("endpoint required") }
    body, err := json.Marshal(env)
    if err != nil { return Envelope{}, fmt.Errorf("marshal envelope: %w", err) }

    client := c.HTTPClient
    if client == nil { client = &http.Client{Timeout: 15 * time.Second} }
    req, err := http.NewRequestWithContext(ctx, http.MethodPost, endpoint, bytes.NewReader(body))
    if err != nil { return Envelope{}, fmt.Errorf("create request: %w", err) }
