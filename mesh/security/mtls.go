...
func (c MTLSConfig) Validate() error {
	...
	if c.MinVersion!=0&&c.MinVersion<tls.VersionTLS13{return errors.New("TLS 1.3 or newer is required")}
...
func (c MTLSConfig) ServerTLS()(*tls.Config,error){cert,pool,err:=LoadMTLSConfig(c);if err!=nil{return nil,err};min:=c.MinVersion;if min==0{min=tls.VersionTLS13};clientAuth:=tls.RequireAndVerifyClientCert;if !c.RequireClient{clientAuth=tls.VerifyClientCertIfGiven};// #nosec G402 -- Validate rejects TLS versions below 1.3; zero defaults to TLS 1.3.
cfg:=&tls.Config{MinVersion:min,Certificates:[]tls.Certificate{cert},ClientCAs:pool,ClientAuth:clientAuth};if strings.TrimSpace(c.SPIFFETrustDomain)!=""{cfg.VerifyConnection=spiffeVerifier(pool,c.SPIFFETrustDomain)};return cfg,nil}
func (c MTLSConfig) ClientTLS()(*tls.Config,error){cert,pool,err:=LoadMTLSConfig(c);if err!=nil{return nil,err};min:=c.MinVersion;if min==0{min=tls.VersionTLS13};// #nosec G402 -- Validate rejects TLS versions below 1.3; zero defaults to TLS 1.3.
cfg:=&tls.Config{MinVersion:min,Certificates:[]tls.Certificate{cert},RootCAs:pool,ServerName:c.ServerName};if strings.TrimSpace(c.SPIFFETrustDomain)!=""{cfg.VerifyConnection=spiffeVerifier(pool,c.SPIFFETrustDomain)};return cfg,nil}
