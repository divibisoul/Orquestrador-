...
func canonicalRequest(kind, capability, correlation string, values []float64) map[string]any {
	...
	return map[string]any{"protocol":"soul-mesh/1","contractVersion":"1.1.0","id":protocol.NewTraceID(),"correlationId":correlation,"source":"N01","target":"N07","kind":kind,"capability":capability,"payload":payload,"timestamp":time.Now().UnixMilli()}
}
...
func TestHTTPGatewayPingAndDescribe(t *testing.T) {
	h:=newTestGateway(t)
	ping:=canonicalRequest("request","mesh.ping","trace-ping",nil)
	got,code:=postWire(t,h,ping)
	if code!=http.StatusOK||got.CorrelationID!="trace-ping"||got.Source!="N07"||got.Target!="N01"||got.Kind!="response"||got.Capability!="mesh.ping"||got.ContractVersion!="1.1.0"{t.Fatalf("unexpected ping response: code=%d envelope=%+v",code,got)}
	describe:=canonicalRequest("request","mesh.describe","trace-describe",nil)
	got,code=postWire(t,h,describe)
	if code!=http.StatusOK||got.CorrelationID!="trace-describe"||got.Kind!="response"{t.Fatalf("unexpected describe response: code=%d envelope=%+v",code,got)}
}

func TestHTTPGatewayExecutesRealNeuralCapability(t *testing.T) {
	h:=newTestGateway(t)
	wire:=canonicalRequest("request","neural.forward","trace-neural",[]float64{1,2,3,4,5,6,7,8})
	got,code:=postWire(t,h,wire)
	if code!=http.StatusOK||got.CorrelationID!="trace-neural"||got.Kind!="response"{t.Fatalf("unexpected neural response: code=%d envelope=%+v",code,got)}
	values,ok:=got.Payload["values"].([]any); if !ok||len(values)!=8{t.Fatalf("expected eight neural outputs, got %#v",got.Payload["values"])}
...
func TestHTTPGatewayHMACAuthentication(t *testing.T) {
	t.Setenv("N07_MESH_HMAC_SECRET",testSecret); t.Setenv("N07_MESH_ALLOW_UNAUTH_LOCAL","false")
	h:=newTestGateway(t); h.Secret=testSecret
	legacy:=protocol.MeshEnvelope{Version:protocol.SoulMeshVersion,ContractVersion:protocol.SoulMeshContractVersion,MessageID:protocol.NewTraceID(),Source:"N01",Target:"N07",Timestamp:time.Now().UnixMilli(),Nonce:protocol.NewTraceID(),CorrelationID:"trace-hmac",Type:"PING",Payload:map[string]any{"capability":"mesh.ping"}}
	if err:=protocol.SignHMAC(&legacy,testSecret);err!=nil{t.Fatal(err)}
	wire:=map[string]any{"protocol":"soul-mesh/1","contractVersion":"1.1.0","id":legacy.MessageID,"correlationId":legacy.CorrelationID,"source":"N01","target":"N07","kind":"request","capability":"mesh.ping","payload":map[string]any{},"timestamp":legacy.Timestamp,"nonce":legacy.Nonce,"hmac":legacy.HMAC}
	got,code:=postWire(t,h,wire)
	if code!=http.StatusOK||got.CorrelationID!="trace-hmac"||got.HMAC==""{t.Fatalf("expected signed Mesh response: code=%d envelope=%+v",code,got)}
}
