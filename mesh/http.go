...
func canonicalType(w canonicalWireEnvelope) string { switch canonicalKind(w) { case "request": if strings.EqualFold(canonicalCapability(w),"mesh.ping")||strings.EqualFold(w.Type,"PING") { return "PING" }; return "CAPABILITY_REQUEST"; case "response": return "TASK_RESULT"; case "error": return "ERROR"; default: return "TASK_RESULT" } }
...
func (g *HTTPGateway) respond(w http.ResponseWriter, status int, in protocol.MeshEnvelope, typ string, payload map[string]any) {
	...
	capability := in.Capability(); if capability=="" { if v,ok:=in.Payload["capability"].(string); ok { capability=strings.TrimSpace(v) } }
	messageID:=protocol.NewTraceID(); timestamp:=time.Now().UnixMilli()
...
func (g *HTTPGateway) Handler(w http.ResponseWriter,r *http.Request) {
	...
	capability:=canonicalCapability(wire); if capability=="" { g.respond(w,http.StatusBadRequest,envelope,"ERROR",map[string]any{"error":"capability is required"}); return }
