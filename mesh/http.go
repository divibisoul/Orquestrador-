...
func(g *HTTPGateway)Handler(w http.ResponseWriter,r *http.Request){if r.Method!=http.MethodPost{writeMeshJSON(w,http.StatusMethodNotAllowed,map[string]any{"error":"POST required"});return};if g.Engine==nil{writeMeshJSON(w,http.StatusServiceUnavailable,map[string]any{"error":"N07 engine unavailable"});return};var wire canonicalWireEnvelope;if err:=json.NewDecoder(http.MaxBytesReader(w,r.Body,1<<20)).Decode(&wire);err!=nil{writeMeshJSON(w,http.StatusBadRequest,map[string]any{"error":err.Error()});return};if wire.Protocol==""||wire.ContractVersion==""||canonicalID(wire)==""||wire.CorrelationID==""||wire.Source==""||wire.Target==""||canonicalKind(wire)==""||wire.Timestamp<=0{writeMeshJSON(w,http.StatusBadRequest,map[string]any{"error":"invalid canonical Mesh identity"});return};envelope:=normalizedMeshEnvelope(wire);if err:=envelope.Validate();err!=nil{writeMeshJSON(w,http.StatusBadRequest,gatewayError(envelope,err.Error()));return};if err:=g.authenticateWire(wire,r,envelope);err!=nil{g.respond(w,http.StatusUnauthorized,envelope,"ERROR",map[string]any{"error":"mesh authentication failed"});return};if envelope.Target!="N07"&&envelope.Target!="BROADCAST"{g.respond(w,http.StatusBadRequest,envelope,"ERROR",map[string]any{"error":"target is not N07"});return};capability:=canonicalCapability(wire);if capability==""{g.respond(w,http.StatusBadRequest,envelope,"ERROR",map[string]any{"error":"capability is required"});return}
...
if canonicalKind(wire)=="request"&&(capability=="mesh.discovery"||capability=="mesh.describe"){payload:=map[string]any{"nucleus":"N07","operations":g.Engine.Operations(),"transports":[]string{"IN_PROCESS","LOOPBACK_HTTP","HTTP","REALTIME"}};if g.Federation!=nil{peers:=g.Federation.Snapshot();snapshot:=make([]map[string]any,0,len(peers));for _,peer:=range peers{snapshot=append(snapshot,map[string]any{"nucleus":peer.Nucleus,"url":peer.URL,"capabilities":peer.Capabilities,"healthy":peer.Healthy,"latency_ms":float64(peer.Latency)/float64(time.Millisecond),"in_flight":peer.InFlight})};payload["peers"]=snapshot};g.respond(w,http.StatusOK,envelope,"TASK_RESULT",payload);return}
...
if _,routeErr:=g.Engine.Route(protocol.Message{TraceID:envelope.CorrelationID,Source:envelope.Source,Target:"N07",Type:"command",Operation:capability,Payload:nil}); routeErr==nil {
    ...
    result,err:=g.Engine.ExecuteWithTrace(r.Context(),envelope.CorrelationID,envelope.Source,capability,values,envelope.NestedMetadata());status:=http.StatusOK;if err!=nil{status=http.StatusBadRequest};payload:=map[string]any{"values":result.Payload,"status":result.Status,"execution":"local"};if result.Error!=""{payload["error"]=result.Error};g.respond(w,status,envelope,"TASK_RESULT",payload);return
}
...
remotePayload:=envelope.NestedPayload();if remotePayload==nil{remotePayload=map[string]any{}}
result,err:=g.Federation.Delegate(r.Context(),envelope.CorrelationID,capability,remotePayload);if err!=nil{g.respond(w,http.StatusBadGateway,envelope,"ERROR",map[string]any{"error":err.Error(),"capability":capability});return}
