...
func VerifiedN01N06Catalog() []FusionCapability {
    ...
    consumers := []string{protocol.N07}
    for _, id := range n01FusionCapabilities {
        out = append(out, FusionCapability{ID:id, Owner:protocol.N01, Kind:"native", Consumers:append([]string(nil), consumers...), Composable:true})
    }
    for _, id := range n06FusionCapabilities {
        out = append(out, FusionCapability{ID:id, Owner:protocol.N06, Kind:"capability", Consumers:append([]string(nil), consumers...), Composable:true})
    }
    return out
}

func KnownFusionOwner(capability string) string {
    for _, c := range VerifiedN01N06Catalog() {
        if c.ID == capability { return c.Owner }
    }
    return ""
}
