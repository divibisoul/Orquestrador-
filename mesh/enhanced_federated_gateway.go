...
func(g *EnhancedFederatedGateway)ServeHTTP(w http.ResponseWriter,r *http.Request){
	...
	if err:=json.Unmarshal(body,&wire);err!=nil{r.Body=io.NopCloser(strings.NewReader(string(body)));g.base.ServeHTTP(w,r);return}
	capability:=canonicalCapability(wire)
	if capability!="mesh.supergpu.parallel"&&capability!="supergpu.parallel"{r.Body=io.NopCloser(strings.NewReader(string(body)));g.base.ServeHTTP(w,r);return}
	...
	if err:=g.base.base.authenticateWire(wire,r,envelope);err!=nil{g.base.base.respond(w,http.StatusUnauthorized,envelope,"ERROR",map[string]any{"error":"mesh authentication failed"});return}
	...
	type task struct{ID string;Capability string;Payload map[string]any;Required bool}
	tasks:=make([]task,0,len(raw));seen:=map[string]struct{}{}
