...
func TestEngineContracts(t *testing.T) {
	...
	ce,err:=ex.EstimateCost(context.Background(),plan);if err!=nil{t.Fatal(err)};if ce.EstimatedTime<=0||ce.Confidence<=0||ce.Confidence>1{t.Fatalf("invalid plan estimate %#v",ce)}
	r,err:=ex.Execute(context.Background(),plan.Workloads[0]);if err!=nil{t.Fatal(err)};if ex.GetLastMetrics().Architecture!=r.Metrics.Architecture{t.Fatal("metrics provider did not retain last metrics")};if len(ex.GetMetricsHistory(10))!=1{t.Fatal("metrics history not updated")}
}
