...
func (e *SimulatedExecutor) EstimateCost(ctx context.Context, plan core.Plan) (core.CostEstimate, error) {
	...
	for _, wl := range plan.Workloads {
		...
		if architecture == "" { architecture = ce.Architecture } else if architecture != ce.Architecture { architecture = "mixed" }
	}
	count := float64(len(plan.Workloads))
	return core.CostEstimate{EstimatedTime: total, EstimatedPFLOPS: weightedPF / count, EstimatedMemoryGB: mem, EstimatedEnergy: energy, Architecture: architecture, Confidence: confidence / count}, nil
}
...
func (e *SimulatedExecutor) Execute(ctx context.Context, wl core.Workload) (core.Result, error) {
	...
	sel, err := selection.Select(wl, e.Catalog, e.Mode, "auto", e.Engine.Config.PrecisionFallback)
	if err != nil { return core.Result{WorkloadID: wl.ID, Error: err}, err }
