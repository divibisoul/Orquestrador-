...
func Select(wl core.Workload, catalog []models.PerformanceModel, strategy string, fallback core.Precision) (Selection, error) {
	...
	if len(candidates)==0 {
		return Selection{}, errors.New("no compatible model satisfies memory and precision requirements")
	}
	best:=candidates[0]
	bestScore:=score(wl,best,strategy)
	for _, candidate:=range candidates[1:] {
		s:=score(wl,candidate,strategy)
		if s<bestScore || (s==bestScore && candidate.Model.Name()<best.Model.Name()) { best,bestScore=candidate,s }
	}
	return best,nil
}

func score(wl core.Workload, s Selection, strategy string) float64 {
	t:=float64(s.Model.EstimateTime(core.Workload{ID:wl.ID,Operation:wl.Operation,Precision:s.EffectivePrecision,MatrixSize:wl.MatrixSize,BatchSize:wl.BatchSize,DataBytes:wl.DataBytes,MemoryNeeded:wl.MemoryNeeded,Priority:wl.Priority,Metadata:wl.Metadata}))
