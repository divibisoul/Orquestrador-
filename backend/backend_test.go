...
func TestMapIntentSuperGPUParallel(t *testing.T) {
	...
	if metadata["operation"] != "relu" || metadata["workers"] != "2" || metadata["inputs_json"] == "" {
		t.Fatalf("unexpected metadata: %#v", metadata)
	}
}
...
func TestConfigUsesEnvironment(t *testing.T) {
	...
	cfg := DefaultConfig()
	if cfg.AppToken != "token" || cfg.SupabaseURL == "" || cfg.SupabaseServiceKey != "service" || cfg.Web3StorageToken != "web3" {
		t.Fatalf("environment binding failed: %#v", cfg)
	}
}

func TestSecureEqual(t *testing.T) {
	if secureEqual("abc", "abd") || !secureEqual("abc", "abc") {
		t.Fatal("secureEqual returned unexpected result")
	}
}
