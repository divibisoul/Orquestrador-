...
func main() {
	...
	status := "FAIL"
	if pass == len(results) { status = "PASS" }
...
func executeProbe(ctx context.Context, client *http.Client, source, target peerConfig) e2eResult {
	...
	if err != nil { return e2eResult{Source: source.Nucleus, Target: target.Nucleus, Status: "fail", CorrelationID: correlationID, Error: err.Error()} }
	...
	if err != nil { return e2eResult{Source: source.Nucleus, Target: target.Nucleus, Status: "fail", LatencyMs: latency, CorrelationID: correlationID, Error: err.Error()} }
	defer resp.Body.Close()
	body, _ := io.ReadAll(io.LimitReader(resp.Body, 1<<20))
	var reply map[string]any
	if err := json.Unmarshal(body, &reply); err != nil { return e2eResult{Source: source.Nucleus, Target: target.Nucleus, Status: "fail", HTTP: resp.StatusCode, LatencyMs: latency, CorrelationID: correlationID, Error: "invalid JSON response"} }
...
func loadPeers() []peerConfig {
	...
	if err := json.Unmarshal([]byte(env("SOUL_MESH_PEERS", "{}")), &raw); err != nil { return nil }
	peers := make([]peerConfig, 0, len(raw))
	for nucleus, value := range raw {
		url := ""
		switch item := value.(type) {
		case string: url = item
		case map[string]any: url, _ = item["url"].(string)
		}
		if strings.TrimSpace(url) != "" { peers = append(peers, peerConfig{Nucleus: nucleus, URL: strings.TrimRight(strings.TrimSpace(url), "/")}) }
	}
