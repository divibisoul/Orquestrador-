...
func NewServer(engine *orchestrator.Engine, pfc *prefrontal.Cortex, registry *mesh.Registry) *Server {
	...
	return &Server{Engine: engine, PFC: pfc, Mesh: registry, Metrics: observability.NewMetrics()}
}
...
func (s *Server) health(w http.ResponseWriter, _ *http.Request) {
	if s == nil {
		http.Error(w, "rest server unavailable", http.StatusServiceUnavailable)
		return
	}
...
func (s *Server) plan(w http.ResponseWriter, r *http.Request) {
	...
	var q request
	if json.NewDecoder(r.Body).Decode(&q) != nil || strings.TrimSpace(q.Goal) == "" {
