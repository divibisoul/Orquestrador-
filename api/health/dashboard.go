...
func (d *Dashboard) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	for _, check := range checks {
		if check.Status != "not-configured" { configured++ }
...
func (d *Dashboard) probe(ctx context.Context, nucleus, baseURL, capability string) probeResult {
	...
	if resp.StatusCode == http.StatusOK && valid { status = "healthy" }
	if resp.StatusCode == http.StatusUnauthorized { status = "unauthorized" }
...
func loadPeers() map[string]string {
	...
	if err := json.Unmarshal([]byte(env("SOUL_MESH_PEERS", "{}")), &raw); err != nil { return peers }
	for nucleus, value := range raw {
		switch item := value.(type) {
		case string:
			if strings.TrimSpace(item) != "" { peers[nucleus] = strings.TrimRight(strings.TrimSpace(item), "/") }
		case map[string]any:
			if url, ok := item["url"].(string); ok && strings.TrimSpace(url) != "" { peers[nucleus] = strings.TrimRight(strings.TrimSpace(url), "/") }
		}
	}
	return peers
}
