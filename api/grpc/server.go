...
func (s *Server) CreateWorkflow(_ context.Context, req *WorkflowRequest) (*WorkflowReply, error) {
	if s == nil || s.Engine == nil {
		return nil, status.Error(codes.FailedPrecondition, "orchestrator engine unavailable")
	}
	...
	for _, task := range req.Tasks {
		...
		steps = append(steps, orchestrator.Step{ID: id, Run: func(context.Context) error { return nil }})
	}
	if len(steps) == 0 {
		return nil, status.Error(codes.InvalidArgument, "at least one task required")
	}
	if err := s.Engine.CreateWorkflow(req.WorkflowID, steps); err != nil {
		return nil, status.Error(codes.AlreadyExists, err.Error())
	}
	return &WorkflowReply{WorkflowID: req.WorkflowID, Status: string(orchestrator.Pending)}, nil
}
...
func (s *Server) GetWorkflowStatus(_ context.Context, req *WorkflowRequest) (*WorkflowReply, error) {
	...
	value, err := s.Engine.GetWorkflowStatus(req.WorkflowID)
	if err != nil {
		return nil, status.Error(codes.NotFound, err.Error())
	}
	return &WorkflowReply{WorkflowID: req.WorkflowID, Status: string(value)}, nil
}

var registerJSONOnce sync.Once
...
func Register(server *grpc.Server, impl OrchestratorServer) error {
	...
	if impl == nil {
		return errors.New("nil orchestrator service")
	}
	registerJSONOnce.Do(func() { encoding.RegisterCodec(jsonCodec{}) })
...
func Serve(lis net.Listener, engine *orchestrator.Engine) (*grpc.Server, error) {
	...
	if err := Register(server, NewServer(engine)); err != nil {
		return nil, err
	}
...
func createWorkflowHandler(srv interface{}, ctx context.Context, dec func(interface{}) error, interceptor grpc.UnaryServerInterceptor) (interface{}, error) {
	...
	if interceptor == nil {
		return srv.(OrchestratorServer).CreateWorkflow(ctx, in)
	}
