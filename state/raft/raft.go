...
func (n *Node) AppendLocal(command []byte)(LogEntry,error){if len(command)==0{return LogEntry{},errors.New("command required")};n.mu.Lock();defer n.mu.Unlock();if n.role!=Leader{return LogEntry{},errors.New("not leader")};e:=LogEntry{Term:n.currentTerm,Index:uint64(len(n.log)+1),Command:append([]byte(nil),command...)};n.log=append(n.log,e);return e,nil}
