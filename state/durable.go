...
func OpenDurable(path string) (*DurableStore, error) {
	...
	if file.Values != nil { s.values = cloneValues(file.Values) }
	if file.Versions != nil { s.versions = file.Versions }
	return s, nil
}

func (s *DurableStore) Put(key string, value []byte) (uint64, error) {
	if s == nil { return 0, errors.New("nil durable store") }
	if key == "" { return 0, errors.New("key required") }
	s.mu.Lock(); defer s.mu.Unlock()
	s.values[key] = append([]byte(nil), value...)
	s.versions[key]++
	if err := s.flushLocked(); err != nil { return 0, err }
	return s.versions[key], nil
}

func (s *DurableStore) Get(key string) ([]byte, uint64, bool) {
	if s == nil { return nil, 0, false }
...
func (s *DurableStore) Delete(key string) error {
	if s == nil { return errors.New("nil durable store") }
	s.mu.Lock(); defer s.mu.Unlock()
	delete(s.values,key); delete(s.versions,key)
	return s.flushLocked()
}

func (s *DurableStore) flushLocked() error {
	file := durableFile{Values:cloneValues(s.values),Versions:map[string]uint64{}}
	for k,v := range s.versions { file.Versions[k]=v }
	data, err := json.Marshal(file); if err != nil { return err }
	tmp := s.path + ".tmp"
