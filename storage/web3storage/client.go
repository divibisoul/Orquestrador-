...
// Config controls the Web3 Storage HTTP adapter. Credentials are intentionally
// supplied at runtime and are never persisted in the repository.
type Config struct {
	APIBaseURL string
	Token      string
...
func ConfigFromEnv() Config {
	...
	return Config{
		APIBaseURL: base,
		Token:      strings.TrimSpace(os.Getenv("WEB3_STORAGE_TOKEN")),
		Timeout:    timeout,
		MaxUpload:  maxUpload,
	}
}
...
// Client is a small transport-neutral adapter around the web3.storage HTTP API.
// The application can replace it with another content-addressed backend later
// without changing Mesh or orchestration contracts.
type Client struct {
	baseURL   string
	token     string
	http      *http.Client
...
// Upload stores a single file and returns its content identifier. The request
// is streamed through multipart/form-data and never exposes the token to logs.
func (c *Client) Upload(ctx context.Context, name string, size int64, r io.Reader) (string, error) {
	...
	if _, err := io.CopyN(part, r, size); err != nil && !errors.Is(err, io.EOF) {
...
// Status returns provider metadata for a CID.
...
// List returns the authenticated account's uploads. It is intentionally kept
// as a provider operation rather than being exposed directly to every nucleus.
...
// Healthy performs an authenticated provider check without uploading data.
