...
type Fabric struct { mu sync.RWMutex; cfg trinity.FabricConfig; tree *DecisionTree }

func NewFabric(cfg trinity.FabricConfig) *Fabric {
	if cfg.DecisionTreeDepth < 1 { cfg.DecisionTreeDepth = 6 }
	if cfg.LearningRate <= 0 { cfg.LearningRate = 0.01 }
	if cfg.FeedbackDiscount <= 0 || cfg.FeedbackDiscount > 1 { cfg.FeedbackDiscount = 0.9 }
...
func (f *Fabric) Route(ctx context.Context, strategy trinity.Strategy, w trinity.Workload) (trinity.Route,error) {
	if f == nil { return trinity.Route{}, errors.New("nil neural fabric") }
	...
	f.mu.RLock(); tree:=f.tree; f.mu.RUnlock()
