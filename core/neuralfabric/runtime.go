...
func (r *Runtime) Route(ctx context.Context,s State,candidates []Route)(Route,error){if err:=contextErr(ctx);err!=nil{return Route{},err};r.mu.RLock();weights:=r.weights;r.mu.RUnlock();if total:=weights[0]+weights[1]+weights[2]+weights[3]+weights[4];total<=0{weights=[5]float64{.35,.2,.2,.15,.1}};type scored struct{r Route;score float64};x:=make([]scored,0,len(candidates));for _,a:=range candidates{l:=r.Latency(s,a);c:=r.Cost(s,a);e:=r.Energy(s,a);q:=r.Quality(s,a);f:=r.Failure(s,a);score:=(weights[0]/.35)*.35*l.Value+(weights[1]/.2)*.2*c.Value+(weights[2]/.2)*.2*e.Value+(weights[3]/.15)*2*(1-q.Value)+(weights[4]/.1)*10*f.Value;a.Score=score;a.Confidence=(l.Confidence+c.Confidence+q.Confidence)/3;x=append(x,scored{a,score})};if len(x)==0{return Route{},errors.New("no route candidates")};sort.Slice(x,func(i,j int)bool{return x[i].score<x[j].score});return x[0].r,nil}
