...
func TestCognitiveLoop(t *testing.T) {
	...
	if err != nil || r.DeviceID != d.ID {
		t.Fatalf("route failed: %v", err)
	}

	res, err := fabric.Execute(ctx, compute.Job{ID: "test", Tokens: 1, Precision: compute.INT8}, d)
	if err != nil || res.JobID != "test" {
		t.Fatalf("compute failed: %v", err)
	}

	cx.OptimizePolicy(0.1)
	agi := superagi.NewRuntime()
	if _, err := agi.GenerateEmbedding(ctx, "hello"); err != nil {
		t.Fatal(err)
	}

	e := orchestrator.NewEngine(2)
	if err := e.CreateWorkflow("w", []orchestrator.Step{{ID: "s", Run: func(context.Context) error { return nil }}}); err != nil {
		t.Fatal(err)
	}
	if err := e.ExecuteStep(ctx, "w"); err != nil {
		t.Fatal(err)
	}
}
