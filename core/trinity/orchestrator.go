...
func (o *TrinityOrchestrator) ExecuteTask(ctx context.Context, task interface{}) (result Result, err error) {
	...
	defer func(){ if o!=nil&&o.Metrics!=nil { o.Metrics.Record("trinity",time.Since(start),failed||err!=nil||!result.Success) } }()
	if ctx==nil{return Result{},errors.New("nil context")}
	// Feature flags are checked before dependency construction so the disabled
	// path is guaranteed to remain a no-op for legacy callers.
	if o==nil{return Result{},ErrDisabled}
	if !o.Config.PFCEnabled||!o.Config.FabricEnabled||!o.Config.ComputeEnabled{return Result{},ErrDisabled}
	if o.PFC==nil||o.Fabric==nil||o.Compute==nil{return Result{},errors.New("trinity dependencies incomplete")}
	...
	route,routeErr:=o.Fabric.Route(ctx,decision.Strategy,w);if routeErr!=nil{route=defaultRoute();if strings.EqualFold(o.Config.FallbackMode,"legacy"){return Result{},fmt.Errorf("route: %w",routeErr)}}
	result,err=o.Compute.Execute(ctx,w,route);if err!=nil{feedbackErr:=o.Fabric.Feedback(ctx,Feedback{WorkloadID:w.ID,Route:route,Success:false,Error:err.Error()});if feedbackErr!=nil{return Result{},fmt.Errorf("compute execute: %w; negative feedback: %v",err,feedbackErr)};return Result{},fmt.Errorf("compute execute: %w",err)}
	...
	failed=!result.Success;return result,nil
}

func adaptTaskToWorkload(task interface{})(Workload,error){switch t:=task.(type){case Workload:return t,validateWorkload(t);case *Workload:if t==nil{return Workload{},ErrUnknownTask};return *t,validateWorkload(*t);case Task:w:=Workload{ID:t.ID,Kind:t.Kind,Payload:t.Payload,MatrixSize:t.MatrixSize,BatchSize:t.BatchSize,MemoryNeeded:t.MemoryNeeded,Precision:t.Precision,Metadata:t.Metadata};return w,validateWorkload(w);case *Task:if t==nil{return Workload{},ErrUnknownTask};return adaptTaskToWorkload(*t);case string:s:=strings.TrimSpace(t);if s==""{return Workload{},errors.New("empty task")};return Workload{ID:s,Kind:"text",Payload:s,Precision:"fp32"},nil;default:return Workload{},ErrUnknownTask}}
