...
type AGI struct { mu sync.RWMutex; Runtime *Runtime; Models []Model; ActiveLoRA string; lastError error }
...
func (a *AGI) Classify(text string,categories []string)(string,float64){if strings.TrimSpace(text)==""{return "empty",1};if len(categories)==0{return "",0};lower:=strings.ToLower(text);for _,c:=range categories{if strings.TrimSpace(c)!=""&&strings.Contains(lower,strings.ToLower(c)){return c,1}};return categories[0],0}
...
func (a *AGI) SwapLoRA(name string)error{if a==nil||a.Runtime==nil{return errors.New("nil AGI")};if err:=a.Runtime.SwapLoRA(context.Background(),name);err!=nil{a.setError(err);return err};a.mu.Lock();a.ActiveLoRA=name;a.mu.Unlock();return nil}
...
func (a *AGI) SelectBestModel(maxLatency float64)Model{if a==nil{return Model{}};a.mu.RLock();defer a.mu.RUnlock();best:=Model{};for _,m:=range a.Models{if (maxLatency<=0||m.LatencyMS<=maxLatency)&&m.Score>best.Score{best=m}};return best}
...
func (a *AGI) ExplainInference(input,output string)string{if a==nil||a.Runtime==nil{return ""};ctx:=context.Background();prompt:=fmt.Sprintf("Explain this inference result using only the supplied input and output. Input: %s\nOutput: %s",input,output);out,err:=a.Runtime.GenerateText(ctx,prompt);a.setError(err);if err!=nil{return ""};return out}
...
func (a *AGI) AutoRetry(input string,confidence float64,maxRetries int)string{if a==nil||a.Runtime==nil{return ""};if maxRetries<0{maxRetries=0};attempts:=maxRetries+1;if attempts>5{attempts=5};var out string;var err error;for i:=0;i<attempts;i++{out,err=a.Runtime.AutoRetry(context.Background(),"default",input,confidence);if err==nil{break}};a.setError(err);return out}
