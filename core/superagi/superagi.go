...
func New() *AGI {
	return &AGI{Runtime: NewRuntime(), Models: []Model{{Name: "default", Precision: "FP32", Score: .7, LatencyMS: 10}}}
}

func NewWithProvider(provider TextProvider) *AGI { return New().WithProvider(provider) }
func (a *AGI) WithProvider(provider TextProvider) *AGI { if a==nil{return a}; a.mu.Lock(); if a.Runtime==nil{a.Runtime=NewRuntime()}; a.Runtime.Provider=provider; a.mu.Unlock(); return a }
...
func (a *AGI) Classify(text string,categories []string)(string,float64){if strings.TrimSpace(text)==""{return "empty",1};if len(categories)==0{return "",0};lower:=strings.ToLower(text);for _,category:=range categories{if strings.Contains(lower,strings.ToLower(category)){return category,1}};return categories[0],0}
...
func (a *AGI) PredictLoRADemand()string{if a==nil||a.Runtime==nil{return "default"};a.mu.RLock();defer a.mu.RUnlock();if a.ActiveLoRA!=""{return a.ActiveLoRA};return "default"}
...
func (a *AGI) ReplayExperience(n int)[]string{if a==nil||a.Runtime==nil||n<=0{return nil};batch:=make([]string,n);for i:=range batch{batch[i]=fmt.Sprintf("replay-%d",i)};if err:=a.Runtime.ReplayExperience(context.Background(),batch);err!=nil{a.setError(err);return nil};return batch}
