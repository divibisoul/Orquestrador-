...
func (r *Runtime) SelectBestModel(models []string)string{best:="";bestScore:=-1;for _,m:=range models{s:=strings.ToLower(strings.TrimSpace(m));if s==""{continue};score:=1.0;if strings.Contains(s,"gemini"){score+=3};if strings.Contains(s,"flash"){score+=1.5};if strings.Contains(s,"pro"){score+=1};if strings.Contains(s,"local")||strings.Contains(s,"cpu"){score+=.5};if score>bestScore{best,bestScore=m,score}};return best}
...
func (r *Runtime) AutoRetry(ctx context.Context,model,input string,minConfidence float64)(string,error){if minConfidence<0{minConfidence=0};var lastErr error;for attempt:=0;attempt<3;attempt++{if attempt>0{delay:=time.Duration(1<<uint(attempt-1))*50*time.Millisecond;timer:=time.NewTimer(delay);select{case <-ctx.Done():timer.Stop();return "",ctx.Err();case <-timer.C:}};out,err:=r.Inference(ctx,model,input);if err==nil{return out,nil};lastErr=err};return "",lastErr}
