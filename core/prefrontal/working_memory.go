	...
	for len(m.items) > m.limit {
		...
		for id, item := range m.items {
			...
			if first || value < victimValue {
				victim, victimValue, first = id, value, false
			}
		}
...
// Get refreshes recency under the write lock, making this a real LRU+score cache.
func (m *WorkingMemory) Get(id string) (trinity.Workload, bool) {
	if m == nil {
		return trinity.Workload{}, false
	}
	m.mu.Lock()
	defer m.mu.Unlock()
	item, ok := m.items[id]
...
func (m *WorkingMemory) Snapshot() []trinity.Workload {
	...
	for _, item := range out {
		result = append(result, item.workload)
	}
	return result
}
