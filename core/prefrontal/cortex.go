...
func (c *Cortex) FuseSignals(signals []Signal) State { c.mu.Lock(); defer c.mu.Unlock(); for _,s:=range signals{c.State.Values[s.Name]=s.Value};c.State.Updated=time.Now();return c.State }
...
func (c *Cortex) UpdateBeliefs(errorValue float64){c.mu.Lock();defer c.mu.Unlock();c.State.Confidence*=1-errorValue;if c.State.Confidence<0{c.State.Confidence=0}}
