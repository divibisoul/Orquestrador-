...
func (f *Fabric) Register(n Nucleus) error {
	if f == nil { return errors.New("nil soul fabric") }
	if err := validateNucleus(n); err != nil { return err }
	f.mu.Lock()
	if f.nodes == nil { f.nodes = make(map[NucleusID]Nucleus) }
	...
	meshRegistry := f.mesh
	f.mu.Unlock()
	if meshRegistry == nil { return errors.New("mesh registry unavailable") }
	return meshRegistry.Announce(mesh.Node{ID:string(n.ID), Status:"ready", Capabilities:append([]string(nil), n.Capabilities...), Transports:append([]string(nil), n.Transports...), LastHeartbeat:time.Now()})
}
...
func (f *Fabric) Snapshot() []Nucleus {
	...
	for _, n := range f.nodes { n.Capabilities=append([]string(nil),n.Capabilities...); n.Transports=append([]string(nil),n.Transports...); out=append(out,n) }
...
func (f *Fabric) Dispatch(ctx context.Context, target NucleusID, traceID, goal string, metadata map[string]interface{}) (transport.Envelope,error) {
	if f == nil { return transport.Envelope{},errors.New("nil soul fabric") }
	if ctx == nil { return transport.Envelope{},errors.New("nil context") }
	traceID=strings.TrimSpace(traceID); if traceID=="" { return transport.Envelope{},errors.New("trace id required") }
	goal=strings.TrimSpace(goal); if goal=="" { return transport.Envelope{},errors.New("goal required") }
	f.mu.RLock(); n,ok:=f.nodes[target]; client:=f.httpClient; f.mu.RUnlock()
	...
	if err!=nil { return transport.Envelope{},fmt.Errorf("dispatch %s: %w",target,err) }
	if response.TraceID!="" && response.TraceID!=traceID { return transport.Envelope{},fmt.Errorf("trace mismatch for %s: sent %q received %q",target,traceID,response.TraceID) }
	if response.Source!="" && response.Source!=string(target) { return transport.Envelope{},fmt.Errorf("source mismatch for %s: received %q",target,response.Source) }
	if response.TraceID=="" { response.TraceID=traceID }
	return response,nil
}

func (f *Fabric) Heartbeat(id NucleusID) error { if f==nil||f.mesh==nil{return errors.New("mesh registry unavailable")};return f.mesh.Heartbeat(string(id)) }
func (f *Fabric) MarkStale(maxAge time.Duration) int { if f==nil||f.mesh==nil{return 0};return f.mesh.MarkStale(maxAge) }
