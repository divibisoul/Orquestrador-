...
func (f *Fabric) Register(n Nucleus) error {
	...
	f.nodes[n.ID] = Nucleus{
		ID: n.ID, Repository: n.Repository, Endpoint: n.Endpoint,
		Capabilities: append([]string(nil), n.Capabilities...),
		Transports: append([]string(nil), n.Transports...),
	}
	meshRegistry := f.mesh
	f.mu.Unlock()
	if meshRegistry == nil {
	...
	return meshRegistry.Announce(mesh.Node{
		ID:            string(n.ID),
		Status:        "ready",
		Capabilities:  append([]string(nil), n.Capabilities...),
		Transports:    append([]string(nil), n.Transports...),
...
func (f *Fabric) RegisterDefaults(endpoints map[NucleusID]string) error {
	...
	for _, id := range []NucleusID{N01, N02, N03, N04, N05, N06} {
		if err := f.Register(Nucleus{
			ID: id, Repository: repos[id], Endpoint: strings.TrimSpace(endpoints[id]),
			Capabilities: []string{"inference", "mesh"}, Transports: []string{MeshProtocol},
		}); err != nil {
			return err
		}
	}
...
func (f *Fabric) Dispatch(ctx context.Context, target NucleusID, traceID, goal string, metadata map[string]interface{}) (transport.Envelope, error) {
	...
	request := transport.Envelope{
		...
		EventType: "soul.task.request", TraceID: traceID, Source: "orquestrador", Target: string(target),
		Timestamp: time.Now().UnixMilli(), CompatibleSystems: []string{"SOUL", MeshProtocol},
	...
	if response.Target != "" && response.Target != string(target) {
		return transport.Envelope{}, fmt.Errorf("target mismatch for %s: received %q", target, response.Target)
	}
	if response.TraceID == "" {
