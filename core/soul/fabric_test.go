...
func TestRegisterDefaultsAndDispatch(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if got := r.Header.Get("X-Soul-Event-Type"); got != "soul.task.request" {
	...
	if err := fabric.RegisterDefaults(endpoints); err != nil {
		t.Fatal(err)
	}
	if got := len(fabric.Snapshot()); got != 6 {
		t.Fatalf("nuclei=%d want 6", got)
	}
