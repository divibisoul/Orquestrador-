...
func NewNeocortex(c *Cortex, n NeuralSignalProvider) (*Neocortex, error) {
	if c == nil { return nil, errors.New("cortex is required") }
	if n == nil { return nil, errors.New("neural signal provider is required") }
...
func (n *Neocortex) Evaluate(ctx context.Context, id string, input []float64, risk, cost, urgency, impact float64) (Candidate, error) {
	if ctx == nil { return Candidate{}, errors.New("context is nil") }
	...
	utility := 0.0
	for _, value := range signal { if value < 0 { utility -= value } else { utility += value } }
	if len(signal) > 0 { utility /= float64(len(signal)) }
...
func (n *Neocortex) Commit(candidate Candidate, reason string) (Decision, error) {
	if n == nil || n.cortex == nil { return Decision{}, errors.New("neocortex unavailable") }
	return n.cortex.Commit(candidate, reason)
}

func (n *Neocortex) Health() map[string]any {
	if n == nil || n.cortex == nil { return map[string]any{"status":"degraded","error":"neocortex unavailable"} }
	h := n.cortex.Health()
